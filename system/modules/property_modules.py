...
def calc_power_pairing_indicator(grid_list, team_num):
	"""
	...
	power_pairing_indicator /= float(len(grid_list)*team_num)
	power_pairing_indicator = math.sqrt(power_pairing_indicator)
	return power_pairing_indicator
...
def is_one_sided(team, position, team_num):#0->fair, 1->many govs, -1->many opps
	if team_num == 4:
		sides = ["og", "oo", "cg", "co"]
	elif team_num == 2:
		sides = ["gov", "opp"]
...
def calc_str_str_indicator(selected_lattice_list, team_num):
	str_str_indicator = 0
	for lattice in selected_lattice_list:
		str_str_indicator += (sum([team.ranking for team in lattice.grid.teams])/float(team_num) - lattice.chair.ranking)**2
	str_str_indicator /= float(len(selected_lattice_list))
	str_str_indicator = math.sqrt(str_str_indicator)
	return str_str_indicator

def calc_deleting_effect(grid_list, grid_deleting, pid, cp_pair):
	comparison_list_by_grid = [cp_pair[0](grid) for grid in grid_list if (list(set(grid.teams)&set(grid_deleting.teams))) != [] and grid.get_available(pid)]
	return sum(comparison_list_by_grid)-cp_pair[0](grid_deleting)

def calc_deleting_effect_adj(lattice_list, lattice_deleting, pid, cp_pair):
	comparison_list_by_lattice = [cp_pair[0](lattice) for lattice in lattice_list if ((lattice.grid == lattice_deleting.grid or lattice.chair == lattice_deleting.chair) and not(lattice.chair.active))]

	return sum(comparison_list_by_lattice)-cp_pair[0](lattice_deleting)

def min_grid_comparison1(grid_list, cp_pair):
	min_grid = min(grid_list, key=lambda grid: cp_pair[0](grid))
	return cp_pair[0](min_grid)
...
def next_wins_distribution(wins_distribution, team_num):
	wins_distribution2 = []
	for i in range(100):
		weight_init = int(wins_distribution.count(i)/team_num)
		weight_last = wins_distribution.count(i)-(team_num-1)*int(wins_distribution.count(i)/team_num)
		for j in range(team_num-1):
			wins_distribution2 += [i+j]*weight_init
		wins_distribution2 += [i+team_num-1]*weight_last
