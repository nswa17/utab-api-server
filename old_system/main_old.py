# -*- coding: utf-8 -*-
#bug: a team appears more than 2 times(BP)
#   : when threading, grid 2 times in lattice (IS the root cause the "disavail grids" or threading, or concentration on one func by many threads?) :::: 3 11 13 14, adj2 adj14
#Todo: sort_adjudicators, type of adjudicators(trainee , panel , chair), adjudicators exchange de adj no basho kawaru
...
try:
	...
	interaction_modules.progress("usage: python3 main.py [mode] #modes are: "+str(list(STYLE_CFGS.keys())))
	sys.exit()
	...
	while True:
		...
		if re.match(r'exchange', a):
			try:
				...
				new_matchups = [copy.copy(selected_grid_lists_with_info[data[0]-1][0]), None]
